  ...
  public void run(String... args) throws Exception {
    ...
    if (conversationRepository.count() == 0) {
      try (InputStream inputStream =
          TypeReference.class.getResourceAsStream("/data/conversations.json")) {
    ...
    if (accountRepository.count() == 0) {
      try (InputStream inputStream =
          TypeReference.class.getResourceAsStream("/data/accounts.json")) {
    ...
    if (participantRepository.count() == 0) {
      try (InputStream inputStream =
          TypeReference.class.getResourceAsStream("/data/participants.json")) {
    ...
    if (messageRepository.count() == 0) {
      try (InputStream inputStream =
          TypeReference.class.getResourceAsStream("/data/messages.json")) {
